...
module tb;
  ...
  import acc_env_pkg::*;
  import acc_test_pkg::*;
  import acc_pkg::NGpr, acc_pkg::NWdr;
  ...
  acc_app_intf                 acc_app_intf (.clk(clk), .rst_n(rst_n));
  tl_if                         tl_if       (.clk(clk), .rst_n(rst_n));
  acc_escalate_if              escalate_if (.clk_i (clk), .rst_ni (rst_n));
  pins_if #(NUM_MAX_INTERRUPTS) intr_if     (interrupts);
  assign interrupts[0] = {intr_done};

  acc_key_req_t sideload_key;
  key_sideload_if#(keymgr_pkg::acc_key_req_t) keymgr_if (
    .clk_i        (clk),
    .rst_ni       (rst_n),
  ...
  acc_otp_key_req_t otp_key_req;
  acc_otp_key_rsp_t otp_key_rsp;

  otp_ctrl_pkg::acc_key_t   key;
  otp_ctrl_pkg::acc_nonce_t nonce;
  wire seed_valid;
  ...
  // Initiate push pull interface for the OTP<->ACC connections
  push_pull_if #(
  ...
  // dut
  acc # (
    .RndCnstAccKey(TestScrambleKey),
    .RndCnstAccNonce(TestScrambleNonce),
    .AccPQCEn(AccPQCEn)
  ) dut (
    .clk_i (clk),
    .rst_ni(rst_n),
    ...
    // Correct behaviour of the idle output is ensured by the bound-in acc_idle_checker instance.
    .idle_o(),
    ...
    .acc_otp_key_o(otp_key_req),
    .acc_otp_key_i(otp_key_rsp),
    .keymgr_key_i(sideload_key),
  ...
  assign dut.u_acc_core.i_acc_trace_if.missed_gnt_i.imem_gnt_missed_err = dut.imem_missed_gnt;
  assign dut.u_acc_core.i_acc_trace_if.missed_gnt_i.dmem_gnt_missed_err = dut.dmem_missed_gnt;

  // Connect AppIntf between DUT and UVM vif
  assign acc_app_intf.kmac_data_req = app_req;
  assign app_rsp = acc_app_intf.kmac_data_rsp;

  bind dut.u_acc_core acc_tracer u_acc_tracer(.*, .acc_trace(i_acc_trace_if));

  bind dut.u_acc_core.u_acc_controller.u_acc_loop_controller
    acc_loop_if i_acc_loop_if (
      .clk_i,
      .rst_ni,
  ...
  assign otp_key_cdc_done = dut.u_acc_scramble_ctrl.otp_key_ack;

  bit [31:0] model_insn_cnt;

  acc_core_model #(
    .MemScope     ("..dut"),
    .DesignScope  ("..dut.u_acc_core")
  ) u_model (
    ...
    .init_sec_wipe_done_i(dut.u_acc_core.i_acc_trace_if.initial_secure_wipe_done),
  ...
  // Pull the final PC and the AccModel handle out of the SV model wrapper.
  ...
  acc_insn_cnt_if insn_cnt_if (
   .clk_i            (clk),
   .rst_ni           (rst_n),

   .insn_cnt_i       (dut.insn_cnt),
   .insn_executing_i (dut.u_acc_core.u_acc_controller.insn_executing),
   .stall_i          (dut.u_acc_core.u_acc_controller.stall),
  ...
  `ASSERT(MatchingReqRND_A, dut.u_acc_core.edn_rnd_req_o == edn_rnd_req_model, clk, !rst_n)
  ...
  `ASSERT(MatchingReqURND_A, dut.u_acc_core.edn_urnd_req_o == edn_urnd_req_model,
    clk, !rst_n || model_if.status == acc_pkg::StatusLocked)
  ...
  initial begin
    ...
    uvm_config_db#(virtual acc_app_intf)::set(null, "*env.m_acc_app_agent", "vif", acc_app_intf);
    ...
    uvm_config_db#(virtual acc_trace_if)::set(null, "*.env", "trace_vif",
                                               dut.u_acc_core.i_acc_trace_if);
    uvm_config_db#(virtual acc_loop_if)::set(
      null, "*.env", "loop_vif",
      dut.u_acc_core.u_acc_controller.u_acc_loop_controller.i_acc_loop_if);
    uvm_config_db#(virtual acc_alu_bignum_if)::set(
      null, "*.env", "alu_bignum_vif",
      dut.u_acc_core.u_acc_alu_bignum.i_acc_alu_bignum_if);
    uvm_config_db#(virtual acc_controller_if)::set(
      null, "*.env", "controller_vif",
      dut.u_acc_core.u_acc_controller.i_acc_controller_if);
    ...
    uvm_config_db#(virtual acc_rnd_if)::set(
      null, "*.env", "rnd_vif",
      dut.u_acc_core.u_acc_rnd.i_acc_rnd_if);
  ...
  // needed for sequences that trigger alerts (locking ACC) without telling the model.
  `DV_ASSERT_CTRL("acc_status_assert_en", tb.MatchingStatus_A)

  // We need to turn off DMEM related assertions in the tests where we force internals of DMEM to generate errors
  `DV_ASSERT_CTRL("DMemAsserts", tb.dut.u_acc_core.u_acc_lsu.DMemRValidAfterReq)
  `DV_ASSERT_CTRL("DMemAsserts", tb.dut.u_acc_core.OnlyWriteLoadDataBaseWhenDMemValid_A)
  `DV_ASSERT_CTRL("DMemAsserts", tb.dut.u_acc_core.OnlyWriteLoadDataBignumWhenDMemValid_A)
