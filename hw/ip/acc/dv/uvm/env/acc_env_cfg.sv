...
class acc_env_cfg extends cip_base_env_cfg #(.RAL_T(acc_reg_block));

  // ext component cfgs
  rand acc_model_agent_cfg model_agent_cfg;

  rand acc_sideload_agent_cfg keymgr_sideload_agent_cfg;

  rand otp_key_agent_cfg key_cfg;

  rand acc_app_agent_cfg m_acc_app_agent_cfg;

  virtual clk_rst_if otp_clk_rst_vif;

  `uvm_object_utils_begin(acc_env_cfg)
  `uvm_object_utils_end

  `uvm_object_new

  virtual acc_trace_if      trace_vif;
  virtual acc_loop_if       loop_vif;
  ...
  virtual acc_ssctrl_if     ssctrl_vif;
  ...
  // An AccMemUtil handle for loading ELF files (set by the test in build_phase)
  chandle mem_util;
  ...
  // low, ACC generates a recoverable error. We want to control this knob whenever we want to see
  ...
  // DPI wrapper (in acc_env::build_phase) to tell it where to find the DUT for backdoor loading
  ...
  function void initialize(bit [31:0] csr_base_addr = '1);
    ...
    list_of_alerts = acc_env_pkg::LIST_OF_ALERTS;
    ...
    model_agent_cfg  = acc_model_agent_cfg  ::type_id::create("model_agent_cfg");
    m_acc_app_agent_cfg = acc_app_agent_cfg::type_id::create("m_acc_app_agent_cfg");
    keymgr_sideload_agent_cfg = acc_sideload_agent_cfg::type_id::create(
    ...
    tl_intg_alert_fields[ral.status.status] = acc_pkg::StatusLocked;
  ...
  // a fifty percent chance of being low. That would result with ACC getting a recoverable error
  // half the time.
  ...
  function logic poll_rnd_edn_req();
    logic rnd_req;
    string rnd_req_hier = $sformatf("%s.u_acc_core.u_acc_rnd.edn_rnd_req_o", dut_instance_hier);
  ...
  function logic [38:0] read_imem_word(bit [ImemIndexWidth-1:0]    idx,
                                       logic [127:0]               key,
                                       acc_pkg::acc_imem_nonce_t nonce);
  ...
  function logic [311:0] read_dmem_word(bit [DmemIndexWidth-1:0]    idx,
                                        logic [127:0]               key,
                                        acc_pkg::acc_dmem_nonce_t nonce);
