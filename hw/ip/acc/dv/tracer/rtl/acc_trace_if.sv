...
/**
 * Interface designed to be bound into acc_core and extract out signals useful for the tracer.
 *
 ...
 *    result in situations where this interface simply renames an existing acc_core signal (by
 *    bringing it in via an input and then assigning it to an internal signal).
 * 3. Hierarchical references can only refer to modules inside acc_core, not acc_core itself as
 *    this requires assuming the instance name of acc_core (which could vary from environment to
 *    environment).
 * 4. To use a signal from the acc_core module, name it as an input with the name used in
 *    acc_core.  Whatever binds the interface into acc_core is responsible for connecting these
...
#(
  ...
  input logic [acc_pkg::WdrAw-1:0] rf_bignum_rd_addr_a,
  input logic [acc_pkg::WdrAw-1:0] rf_bignum_rd_addr_b,
  input logic [acc_pkg::WdrAw-1:0] rf_bignum_wr_addr,
  ...
  input logic [1:0][acc_pkg::SideloadKeyWidth-1:0] sideload_key_shares_i,
  ...
  // consistent `insn_X` signals are provided here that are simply assigned to `acc_core` signals.
  ...
  assign insn_stall     = u_acc_core.u_acc_controller.state_d == AccStateStall;
  ...
  assign rf_base_rd_data_a = u_acc_controller.rf_base_rd_data_a_no_intg;
  assign rf_base_rd_data_b = u_acc_controller.rf_base_rd_data_b_no_intg;
  assign rf_base_wr_data = u_acc_rf_base.wr_data_intg_mux_out[31:0];

  assign rf_bignum_rd_data_a = u_acc_controller.rf_bignum_rd_data_a_no_intg;
  assign rf_bignum_rd_data_b = u_acc_controller.rf_bignum_rd_data_b_no_intg;
  ...
  for (genvar i = 0; i < NWdr; ++i) begin : g_probe_bignum_rf
    if (RegFile == RegFileFF) begin : g_rf_ff_probe
      assign bignum_rf[i] = u_acc_rf_bignum.gen_rf_bignum_ff.u_acc_rf_bignum_inner.rf[i];
    end else if (RegFile == RegFileFPGA) begin : g_rf_fpga_probe
      assign bignum_rf[i] = u_acc_rf_bignum.gen_rf_bignum_fpga.u_acc_rf_bignum_inner.rf[i];
    end
  end
  ...
  assign ispr_write[IsprMod] = |u_acc_alu_bignum.mod_wr_en & ~ispr_init;
  ...
  generate
    if (AccPQCEn) begin : gen_ispr_data_pqc
      for (genvar i_word = 0; i_word < BaseWordsPerWLEN; i_word++) begin : g_mod_and_acc_words
        ...
        assign ispr_write_data[IsprAcc][i_word*32+:32] = u_acc_mac_regs.acc_intg_d[i_word*39+:32];
        assign ispr_write_data[IsprKmacMsg][i_word*32+:32] =
          u_acc_alu_bignum.gen_pqc_wsr.kmac_msg_wr_en[i_word] ? u_acc_alu_bignum.gen_pqc_wsr.kmac_msg_intg_d[i_word*39+:32] :
                                                    u_acc_alu_bignum.gen_pqc_wsr.kmac_msg_intg_q[i_word*39+:32];
        assign ispr_read_data[IsprKmacMsg][i_word*32+:32] = u_acc_alu_bignum.gen_pqc_wsr.kmac_msg_intg_q[i_word*39+:32];
        assign ispr_write_data[IsprAccH][i_word*32+:32] =
            u_acc_mac_regs.gen_acch_reg.acch_intg_d[i_word*39+:32];
      end
  ...
  generate
    if (AccPQCEn) begin : gen_ispr_read_write_pqc
      // KMAC MSG
      ...
      // ACCH
      assign ispr_write[IsprAccH]     = u_acc_mac_regs.gen_acch_reg.acch_en & ~ispr_init;
      ...
      assign ispr_read_data[IsprAccH] =
          (any_ispr_read & (ispr_addr == IsprAccH)) ?
            u_acc_mac_regs.gen_acch_reg.acch_no_intg_q :
            u_acc_mac_regs.acch_blanked_o;
  ...
  // ACC
  assign ispr_write[IsprAcc] = u_acc_mac_regs.acc_en & ~ispr_init;
  ...
  assign ispr_read_data[IsprAcc] =
      (any_ispr_read & (ispr_addr == IsprAcc)) ? u_acc_mac_regs.acc_no_intg_q :
                                                 u_acc_mac_regs.acc_blanked_o;
  ...
  for (genvar i_fg = 0; i_fg < NFlagGroups; i_fg++) begin : g_flag_group_acceses
    assign flags_write[i_fg] = (sec_wipe_zero |
        ((u_acc_alu_bignum.alu_predec_bignum_i.flags_adder_update[i_fg] |
          u_acc_alu_bignum.alu_predec_bignum_i.flags_logic_update[i_fg] |
          u_acc_alu_bignum.alu_predec_bignum_i.flags_mac_update[i_fg] |
          (|u_acc_alu_bignum.alu_predec_bignum_i.flags_ispr_wr)) &
          u_acc_alu_bignum.operation_commit_i)) & ~ispr_init;
    assign flags_write_data[i_fg] = u_acc_alu_bignum.flags_d[i_fg];
    ...
    assign flags_read_data[i_fg] = u_acc_alu_bignum.flags_q[i_fg];
  end
  ...
  assign start_stop_bad_int_i.spr_urnd_acks = u_acc_rnd.edn_urnd_ack_i &&
    (!u_acc_rnd.edn_urnd_req_o);
  assign start_stop_bad_int_i.spr_rnd_acks = u_acc_rnd.edn_rnd_ack_i &&
    (!u_acc_rnd.edn_rnd_req_o);
  assign start_stop_bad_int_i.spr_secwipe_reqs = u_acc_start_stop_control.secure_wipe_error_q;

  assign start_stop_bad_int_i.mubi_rma_err = u_acc_start_stop_control.mubi_err_d &&
    prim_mubi_pkg::mubi4_test_invalid(u_acc_start_stop_control.rma_ack_q);
  assign start_stop_bad_int_i.mubi_urnd_err = u_acc_start_stop_control.mubi_err_d &&
    prim_mubi_pkg::mubi4_test_invalid(u_acc_start_stop_control.wipe_after_urnd_refresh_q);

  assign controller_bad_int_i.loop_hw_cnt_err =
    (|u_acc_controller.u_acc_loop_controller.loop_counter_err);

  assign controller_bad_int_i.loop_hw_stack_cnt_err =
    u_acc_controller.u_acc_loop_controller.loop_stack_cnt_err;

  assign controller_bad_int_i.loop_hw_intg_err =
    ((|u_acc_controller.u_acc_loop_controller.current_loop_intg_err) &&
     u_acc_controller.u_acc_loop_controller.current_loop_valid);

  assign controller_bad_int_i.rf_base_call_stack_err =
    u_acc_controller.rf_base_call_stack_hw_err_i;
  assign controller_bad_int_i.spr_secwipe_acks = u_acc_controller.spurious_secure_wipe_ack_q;
  assign controller_bad_int_i.state_err = u_acc_controller.state_error;
  assign controller_bad_int_i.controller_mubi_err = u_acc_controller.mubi_err_q;

  // Probe Start Stop Control module FSM to inject escalate signals in specific states.
  acc_start_stop_state_e acc_start_stop_state;
  // Use this to mute Verilator unused errors.
  acc_start_stop_state_e unused_start_stop_state;

  assign acc_start_stop_state = u_acc_start_stop_control.state_q;
  assign unused_start_stop_state = acc_start_stop_state;
  ...
  function static void force_rf_bignum_rd_data_a_intg(
      input logic [acc_pkg::ExtWLEN-1:0] should_val
    );
    force u_acc_rf_bignum.rd_data_a_intg_o = should_val;
  endfunction
