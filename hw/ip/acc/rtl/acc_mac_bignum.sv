...
#(
  ...
  input logic [WLEN-1:0] acch_blanked_i,
  input logic [WLEN-1:0] acc_blanked_i,
  ...
  generate
    if (!AccPQCEn) begin : gen_unused_outputs
      ...
      assign unused_bits = ^{operation_i.sel, operation_i.lane_mode, operation_i.lane_word_32,
                             ...
                             operation_i.data_type, acch_blanked_i};
    end
  endgenerate
  ...
  generate
    if (AccPQCEn) begin : gen_adder_op_pqc
      // Add shifted multiplier result to current accumulator.
      assign adder_op_b = {acch_blanked_i, acc_blanked_i};
    end else begin : gen_adder_op
      // Add shifted multiplier result to current accumulator.
      assign adder_op_b = acc_blanked_i;
    end
  endgenerate
  ...
  generate
    if (AccPQCEn) begin : gen_op_result_pqc
      always_comb begin
        case (operation_i.mulv)
          1'b0 : begin
